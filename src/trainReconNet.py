...
def main():
    ...
    # Data Loading
    data_dir = '/home/jonzamora/Desktop/m2cai16-tool/train_dataset'

    image_datasets = {x: miccaiDataset(os.path.join(data_dir, x), data_transforms[x]) for x in ['train', 'test']}
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, img in enumerate(train_loader):
        ...
        print('[%d/%d][%d/%d] Total-Loss: %.4f'
              % (epoch, args.epochs-1, i, len(train_loader)-1, loss.mean().detach()))
...
def validate(val_loader, model, criterion, epoch):
    ...
    for i, img in enumerate(val_loader):
        ...
        print('[%d/%d][%d/%d] Total-Loss: %.4f'
              % (epoch, args.epochs-1, i, len(val_loader)-1, loss.mean().detach()))
